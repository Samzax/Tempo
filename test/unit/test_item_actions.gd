...
func test_health_component_heal_clamps_and_ignores_non_positive_amounts() -> void:
	...
	health.heal(1.0)
	assert_eq(health.health, 2.0, "Positive healing should be applied.")
	health.heal(5.0)
	assert_eq(health.health, 3.0, "Healing should not exceed max health.")
	health.heal(0.0)
	health.heal(-2.0)
	assert_eq(health.health, 3.0, "Zero and negative healing should be ignored.")
...
func test_heal_action_ignores_null_context_and_null_owner() -> void:
	...
	sentinel_owner.health.max_health = 3.0
	sentinel_owner.health.health = 1.0
	...
	assert_eq(sentinel_owner.health.health, 1.0, "Null context and owner must leave valid owner state unchanged.")
...
func test_damage_action_uses_payload_center_and_copies_canonical_damage_info() -> void:
	...
	assert_eq(payload_side_enemy.received.amount, 7.0)
