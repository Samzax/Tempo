...
func test_engineer_deployable_propagates_partial_and_lethal_damage() -> void:
	...
	assert_eq(deployable.take_damage(_damage(1.25)), HEALTH_UNITS.from_hp(1.25))
	assert_eq(deployable.health.health, HEALTH_UNITS.from_hp(4.75))
	assert_eq(deployable.take_damage(_damage(20.0)), HEALTH_UNITS.from_hp(4.75))
	assert_eq(deployable.take_damage(_damage(1.0)), 0)
...
func test_engineer_deployable_blocks_invalid_damage_without_mutation() -> void:
	...
	add_child_autofree(deployable)
	await get_tree().process_frame
	var before: int = deployable.health.health

	for info in [null, _damage(0.0), _damage(-1.0), _damage(NAN), _damage(INF)]:
		assert_eq(deployable.take_damage(info), 0)
...
func test_trap_burst_and_aoe_detonate_and_return_zero() -> void:
	for tag in [&"burst", &"aoe"]:
		...
		assert_eq(trap.take_damage(info), 0)
